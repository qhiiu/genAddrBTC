...
class Secp256K1
{
	public:
		...
		void GetHash160(bool isCompressed, Point& pubKey, unsigned char* hash);
...
void Secp256K1::GetHash160(bool isCompressed, Point& pubKey, unsigned char* hash)
{
...
class Hiiu 
{
	public:
		...
		void privToHash160(char* p_hex, uint32_t* _hash160, bool isCompressed);
		std::string privToAddr(char* p_hex, bool isCompressed); // return STRING

		void pubkeyToHash160(Point& pubKey, uint32_t* _hash160, bool isCompressed); 
		std::string pubkeyToAddr(Point& pubKey, bool isCompressed); //return STRING 
...
//--------------------------------------------------------------------
 ...
 void Hiiu::privToHash160(char* p_hex, uint32_t* _hash160, bool isCompressed)
 {
	...
	hiiu_secp->GetHash160(isCompressed, pubKey, address + 1); // 1 compressed
...
std::string Hiiu::privToAddr(char* p_hex, bool isCompressed)
{
	char* priv_hex = p_hex;   // char* priv_hex = "2832ed74f2b5e35ee"; 
	...
	hiiu_secp->GetHash160(isCompressed, pubKey, address + 1);
...
//--------------------------------------------------------------------
void Hiiu::pubkeyToHash160(Point& pubKey, uint32_t* _hash160,  bool isCompressed)
{
	Secp256K1* hiiu_secp = new Secp256K1();   
...
//--------------------------------------------------------------------
std::string Hiiu::pubkeyToAddr(Point& pubKey, bool isCompressed)
{
	Secp256K1*	hiiu_secp = new Secp256K1();   
	hiiu_secp->Init();	
...
int main(){
	...
	//pubkey --> Hash160
	uint32_t hash160[5]; 
	hiiu.pubkeyToHash160(publicKey, hash160, isCompressed);
			// for (int i = 0; i < 5; i++){	printf("\n --- _hash160[] : %d ", get_h[i]);				}

	//hash160 --> Addr
	std::string addrz = hiiu.hash160ToAddr(hash160, isCompressed);
	std::cout << "\naddrz : "<< addrz; 
	
	//priv --> Addr
	std::string addr = hiiu.privToAddr("2832ed74f2b5e35ee", isCompressed);
	std::cout << std::endl << "addr : " << addr;

	std::cout << "\n\n -------------------------------------------- \n";

	//priv --> hash160 
	uint32_t h[5]; 
	hiiu.privToHash160("2832ed74f2b5e35ee", h, isCompressed);
	std::cout<<"\n\n -------------------------------------------- \n";

	// pubkey --> addr 
	std::string addr1 = hiiu.pubkeyToAddr(publicKey, isCompressed);
	std::cout << "\n addr1 : "<<addr1;
